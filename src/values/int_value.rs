...
impl<'ctx> IntValue<'ctx> {
    ...
    pub fn const_not(self) -> Self {
        let value = unsafe { LLVMConstNot(self.as_value_ref()) };
    ...
    pub fn const_nsw_neg(self) -> Self {
        let value = unsafe { LLVMConstNSWNeg(self.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nuw_neg(self) -> Self {
        let value = unsafe { LLVMConstNUWNeg(self.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_add(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstAdd(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nsw_add(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNSWAdd(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nuw_add(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNUWAdd(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_sub(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstSub(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nsw_sub(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNSWSub(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nuw_sub(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNUWSub(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_mul(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstMul(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nsw_mul(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNSWMul(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_nuw_mul(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstNUWMul(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_unsigned_div(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstUDiv(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_signed_div(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstSDiv(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_exact_signed_div(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstExactSDiv(self.as_value_ref(), rhs.as_value_ref()) };
    ...
    pub fn const_exact_unsigned_div(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstExactUDiv(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_unsigned_remainder(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstURem(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_signed_remainder(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstSRem(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_and(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstAnd(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_or(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstOr(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_xor(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstXor(self.as_value_ref(), rhs.as_value_ref()) };
    ...
    pub fn const_shl(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstShl(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_rshr(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstLShr(self.as_value_ref(), rhs.as_value_ref()) };

        IntValue::new(value)
    }

    pub fn const_ashr(self, rhs: IntValue<'ctx>) -> Self {
        let value = unsafe { LLVMConstAShr(self.as_value_ref(), rhs.as_value_ref()) };
    ...
    pub fn const_unsigned_to_float(self, float_type: FloatType<'ctx>) -> FloatValue<'ctx> {
        let value = unsafe { LLVMConstUIToFP(self.as_value_ref(), float_type.as_type_ref()) };
    ...
    pub fn const_signed_to_float(self, float_type: FloatType<'ctx>) -> FloatValue<'ctx> {
        let value = unsafe { LLVMConstSIToFP(self.as_value_ref(), float_type.as_type_ref()) };

        FloatValue::new(value)
    }

    pub fn const_to_pointer(self, ptr_type: PointerType<'ctx>) -> PointerValue<'ctx> {
        let value = unsafe { LLVMConstIntToPtr(self.as_value_ref(), ptr_type.as_type_ref()) };

        PointerValue::new(value)
    }

    pub fn const_truncate(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstTrunc(self.as_value_ref(), int_type.as_type_ref()) };
    ...
    pub fn const_s_extend(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstSExt(self.as_value_ref(), int_type.as_type_ref()) };
    ...
    pub fn const_z_ext(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstZExt(self.as_value_ref(), int_type.as_type_ref()) };

        IntValue::new(value)
    }

    pub fn const_truncate_or_bit_cast(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstTruncOrBitCast(self.as_value_ref(), int_type.as_type_ref()) };
    ...
    pub fn const_s_extend_or_bit_cast(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstSExtOrBitCast(self.as_value_ref(), int_type.as_type_ref()) };
    ...
    pub fn const_z_ext_or_bit_cast(self, int_type: IntType<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstZExtOrBitCast(self.as_value_ref(), int_type.as_type_ref()) };

        IntValue::new(value)
    }

    pub fn const_bit_cast(self, int_type: IntType) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstBitCast(self.as_value_ref(), int_type.as_type_ref()) };
    ...
    pub fn const_int_compare(self, op: IntPredicate, rhs: IntValue<'ctx>) -> IntValue<'ctx> {
        let value = unsafe { LLVMConstICmp(op.into(), self.as_value_ref(), rhs.as_value_ref()) };
